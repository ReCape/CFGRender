...
const texture = loader.load ('texture2.png', loadCfg)
...
skin.wrapS = THREE.Repeat;
skin.wrapT = THREE.Repeat;
...
function isSkinSlim() {
    var canvasAndContext = textureToCanvas(skin);
...
function loadOuterLayer(skinPart) {
    skinPartName = skinPart[0]
        ...
        const material = new THREE.MeshStandardMaterial({map:skin, transparent:true});
...
function loadPlayer() {
    ...
    Object.entries(loadingParts).forEach((skinPart) => {
        ...
        const material = new THREE.MeshStandardMaterial({map:skin});
        ...
        loadOuterLayer([skinPartName,skinPart])
    })
}
